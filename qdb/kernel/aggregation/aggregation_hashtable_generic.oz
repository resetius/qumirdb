(block
  ...
  (fun rh_lookup_slot ((var keys <ptr <named Key (template readable mutable)>>)
                       (var dist <ptr i64>)
  ...
  (fun rh_insert_displace ((var keys <ptr <named Key (template readable mutable)>>)
                           (var dist <ptr i64>)
                           (var slot_ids <ptr i64>)
                           (var capacity i64)
  ...
  (fun aht_destroy ((var ht <ref HashTable>))
    (block
      ...
      (var owned_blocks = (field ht OwnedBlocks))
      (var owned_count = (field ht OwnedBlockCount))
      (var owned_index i64)
      (= owned_index (: 0 i64))
      (while (< owned_index owned_count)
        (block
          (call qdb_free (cast (index owned_blocks owned_index) <ptr i8>))
          (= owned_index (+ owned_index (: 1 i64)))))
      (if (!= (cast owned_blocks i64) (: 0 i64))
        (block (call qdb_free (cast owned_blocks <ptr i8>))))
    ...
    (block
      ...
      (if allocation_failed
        (block
          ...
          (return #f)))
      (var old_keys_typed =
        (cast (field ht Keys) <ptr <named Key (template readable mutable)>>))
      (var new_keys_typed =
        (cast new_keys <ptr <named Key (template readable mutable)>>))
      (if (! (call rh_rehash_into old_keys_typed (field ht Dist) (field ht SlotId)
        ...
        (block
          ...
          (return #f)))
      (var old_group_typed =
        (cast (field ht GroupKeys) <ptr <named Key (template readable mutable)>>))
      (var new_group_typed =
